...
def BFS(graph, start, end):
    queue = [(start,[start])]; visited = set(); print("[INF] Performing a Breadth-first search traversal...\n")
    while queue:
    ...
    while not instancePositions.intersection(graph):
        for cube in toRotate: 
            ...
            for instance in instancePositions: 
                if instance in graph: print("[INF] Found instance after {:,} permutations\n".format(len(graph))); return (BFS(graph, solvedCube, instance))
        toRotate = temporary; temporary = set()

# 6 lines
def printSolution(solution):
    steps = len(solution) - 1; print("[INF] Steps to solve your cube:\n")
    ...
    print("[SOLVED] Minimum number of steps needed " + grammar)
